    ...
    <Layout>
      <div className="mb-8 animate-fade-in">
        <h1 className="text-4xl font-black text-white tracking-tight">
          Staff <span className="text-accent-400">Dashboard</span>
        </h1>
        <p className="text-gray-500 mt-2 font-medium text-lg">Welcome, {user?.fullName}. Manage your assigned requests.</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-slide-up">
        {[
          { label: 'Assigned to Me', value: stats.total, icon: <ClipboardList size={22} />, color: 'blue' },
      ...
      {/* Filter */}
      <div className="flex bg-dark-800/60 p-1 rounded-xl border border-surface-border overflow-x-auto w-full sm:w-min mb-6 animate-slide-up" style={{ animationDelay: '100ms' }}>
      ...
      <div className="animate-slide-up" style={{ animationDelay: '150ms' }}>
        {loading ? (
          ...
          <div className="dark-card text-center py-16">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No assigned requests</h3>
            <p className="text-gray-500 max-w-sm mx-auto">You don't have any requests matching this filter.</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-gray-500">
                      <span className="flex items-center gap-1.5 bg-dark-700/60 px-2.5 py-1 rounded-lg">👤 {c.studentName}</span>
                      <span className="flex items-center gap-1.5 bg-dark-700/60 px-2.5 py-1 rounded-lg">📂 {c.categoryName}</span>
                      <span className="flex items-center gap-1.5 bg-dark-700/60 px-2.5 py-1 rounded-lg">
                        📅 {new Date(c.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                  </div>
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 sm:border-l border-surface-border pt-3 sm:pt-0 sm:pl-6">
